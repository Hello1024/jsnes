...
(function(global) {
  ...
  var FullyConnLayer = function(opt) {
    ...
    // computed
    this.num_inputs = (opt.preserve_sx?1:opt.in_sx) * (opt.preserve_sy?1:opt.in_sy) * opt.in_depth;
    this.out_sx = opt.preserve_sx ? opt.in_sx : 1;
    this.out_sy = opt.preserve_sy ? opt.in_sy : 1;
    this.layer_type = 'fc';
  ...
  FullyConnLayer.prototype = {
    forward: function(V, is_training) {
      ...
      var A = new Vol(this.out_sx, this.out_sy, this.out_depth, 0.0);
      var Vw = V.w;
      for(var xy=0; xy<this.out_sx*this.out_sy; xy++) {
        var Vw_xy = xy*this.num_inputs;
        var Aw_xy = xy*this.out_depth;
        
        for(var i=0;i<this.out_depth;i++) {
          var a = 0.0;
          var wi = this.filters[i].w;
          for(var d=0;d<this.num_inputs;d++) {
            a += Vw[Vw_xy+d] * wi[d]; // for efficiency use Vols directly for now
          }
          a += this.biases.w[i];
          A.w[Aw_xy+i] = a;
        }
    ...
    backward: function() {
      ...
      for(var xy=0; xy<this.out_sx*this.out_sy; xy++) {
        ...
        for(var i=0;i<this.out_depth;i++) {
          ...
          var chain_grad = this.out_act.dw[Aw_xy+i];
          for(var d=0;d<this.num_inputs;d++) {
            V.dw[Vw_xy+d] += tfi.w[d]*chain_grad; // grad wrt input data
            tfi.dw[d] += V.w[Vw_xy+d]*chain_grad; // grad wrt params
          }
...
(function(global) {
  ...
  // This "layer" grabs data from many past layers and stacks it.
  ...
  // It does nearest neighbour resizing on any layer not the right
  // dimensions.  It injects two more layers for position information.
  // The corner layer has no weights.

  
  var CornerLayer = function(opt) {
    var opt = opt || {};

    // required
    this.in_layers = opt.in_layers;

    var cornerthis = this;
    this.in_size = 0;
    this.in_layers.forEach(function(layer) {
      var cur_size = cornerthis.in_size;
      layer.consumers.push(function(l) {
        cornerthis.backskip(l, cur_size);
      });

      cornerthis.in_size += layer.out_depth;
    });

    this.out_depth = this.in_size+2;
    this.out_sx = opt.sx;
    this.out_sy = opt.sy;
    
    this.layer_type = 'corner';

  };
  CornerLayer.prototype = {
    forward: function(V, is_training) {
      this.in_act = V;

      var A = new Vol(this.out_sx, this.out_sy, this.out_depth, 0.0);
      var d=0;
      for (var layer_num=0; layer_num<this.in_layers.length; layer_num++) {
        var layer = this.in_layers[layer_num];
        for (var x=0; x<this.out_sx; x++) {
          for (var y=0; y<this.out_sy; y++) {
            var xy = (x+y*this.out_sx) * this.out_depth;
            var in_xy = (Math.floor(x/this.out_sx*layer.out_sx)
                       + Math.floor(y/this.out_sy*layer.out_sy)*layer.out_sx)
                       *layer.out_depth;
            for (var in_d=0; in_d<layer.out_depth; in_d++) {                
              A.w[xy+d+in_d] = layer.out_act.w[in_xy+in_d];
            }
      ...
      // add dimension layers.
      for (var x=0; x<this.out_sx; x++) {
        for (var y=0; y<this.out_sy; y++) {
          var xy = (x+y*this.out_sx) * this.out_depth;
          A.w[xy + d] = x / this.out_sx;  // dimension
          A.w[xy + d + 1] = y / this.out_sy;
        }
      }
      this.out_act = A;
      return this.out_act;
    },
    backskip: function(layer, d) {

      // just propogate (adding to not clobber existing data)
      // dw's back to previous steps.
      var V = layer.in_act;
      for (var x=0; x<this.out_sx; x++) {
        for (var y=0; y<this.out_sy; y++) {
          var xy = (x+y*this.out_sx)*this.out_depth;
          var in_xy = (Math.floor(x/this.out_sx*layer.out_sx)
                     ...
                     *layer.out_depth;
          for (var in_d=0; in_d<layer.out_depth; in_d++) {
            V.dw[in_xy+in_d] += this.out_act.dw[d+xy+in_d]
          }
        }
      }
    },
    backward: function() {},   // do nothing, this doesn't go back!
    getParamsAndGrads: function() {
      var response = [];
      return response;
    },
  ...
  global.CornerLayer = CornerLayer;
  
  // noop layer, but notifies anyone consuming data from elsewhere
  // when a backprop occurs.  (consumer must be ahead to prevent loops)
  var SplitLayer = function(opt) {
    var opt = opt || {};
    ...
    this.layer_type = 'split';

    // initialise
    this.consumers = []  // List of people using data from this layer
                         // following layer is implicit.
  }
  SplitLayer.prototype = {
    
    forward: function(V, is_training) {
      return this.in_act = this.out_act = V;
    },
    backward: function() {
      // notify the consumers.  They will update weights for us.
      for(var i=0; i<this.consumers.length; i++)
        this.consumers[i](this);
    },
    getParamsAndGrads: function() {
      var response = [];
      return response;
    ...
    toJSON: function() {
      ...
      json.out_depth = json.in_depth = this.out_depth;
      json.out_sx = json.in_sx = this.out_sx;
      json.out_sy = json.in_sy = this.out_sy;
      return json;
    },
    fromJSON: function(json) {
      this.out_depth = this.in_depth = json.out_depth;
      this.out_sx = this.in_sx = json.out_sx;
      this.out_sy = this.in_sy = json.out_sy;
    }
  }
  global.SplitLayer = SplitLayer;
  ...
  Net.prototype = {
    ...
    makeLayers: function(defs) {
      ...
      for(var i=0;i<defs.length;i++) {
        ...
        switch(def.type) {
          ...
          case 'corner': 
            def.in_layers = corner_layers;
            this.layers.push(new global.CornerLayer(def));
            break;
          case 'split': 
            this.layers.push(new global.SplitLayer(def));
            corner_layers.push(this.layers.slice(-1)[0]);
            break;
    ...
    fromJSON: function(json) {
      ...
      for(var i=0;i<json.layers.length;i++) {
        ...
        if(t==='corner') { L = new global.CornerLayer(); }
        L.fromJSON(Lj);
        this.layers.push(L);
      }
    }
